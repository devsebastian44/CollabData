...
export const users: { [key: string]: User } = {
  ...
  user4: {
    name: 'David',
    avatarUrl: PlaceHolderImages.find((p) => p.id === 'user-4')?.imageUrl || '',
  },
  user5: {
    name: 'Eve',
    avatarUrl: PlaceHolderImages.find((p) => p.id === 'user-5')?.imageUrl || '',
  },
};
...
export const files: DatasetFile[] = [
  {
    ...
    size: '24MB',
    status: 'Synced',
    icon: React.createElement(FileText, { className: 'text-green-400' }),
    isActive: true,
  },
  {
    name: 'Customer_Survey_2023.xlsx',
    type: 'xlsx',
    size: '2MB',
    status: 'Local',
    icon: React.createElement(Table, { className: 'text-blue-400' }),
  },
];
...
export const referenceFiles: DatasetFile[] = [
  {
    ...
    size: '14KB',
    status: 'Synced',
    icon: React.createElement(Braces, { className: 'text-yellow-400' }),
  },
  {
    name: 'Historical_Log_2022.csv',
    type: 'csv',
    size: '156MB',
    status: 'Archived',
    icon: React.createElement(FileText, { className: 'text-green-400' }),
